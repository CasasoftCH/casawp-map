							...
							$i = 0; foreach ($cols as $col) { $i++;
								switch ($col['field']) {
									case 'status':
										$value = '';
										switch ($status) {
											case 'available': $value = '<span class="text-success">'.__('Available', 'casasyncmap').'</span>'; break;
											case 'reserved': $value = '<span class="text-'.$state.'">'.__('Reserved', 'casasyncmap').'</span>'; break;
