...
<form id="complexContactFormAnchor" class="complex-contact-form" action="#complexContactFormAnchor" method="POST">
	...
	<div class="row">
		<div class="col-md-2">
			<?php _e('I am interested in the unit', 'casasyncmap') ?>
				...
				<dd class="editable">
					<div class="row">
						<div class="col-xs-6 complex-form-gender-m">
							<div class="radio">
								<label>
									<input type="radio" name="complex-unit-inquiry[gender]" value="male" <?= ($data['gender'] == 'male' ? 'checked="checked"' : '') ?>> <?php _e('Mr.', 'casasyncmap') ?>
								</label>
							</div>
						</div>
						<div class="col-xs-6 complex-form-gender-fm">
							<div class="radio">
								<label>
									<input type="radio" name="complex-unit-inquiry[gender]" value="female" <?= ($data['gender'] == 'female' ? 'checked="checked"' : '') ?>> <?php _e('Mrs.', 'casasyncmap') ?>
								</label>
							</div>
						</div>
					</div>
			...
			<dl>
				...
				<dd class="editable">
					<div class="<?= (isset($messages['subject'])  ? 'has-error' : '') ?>">
						<input type="text" name="complex-unit-inquiry[subject]" rows="7" class="form-control" placeholder="<?php _e('What do you wish to know?', 'casasyncmap') ?>" value="<?= $data['subject'] ?>" />		
					</div>
				</dd>
				<dt class="editable">
					<?php _e('Message', 'casasyncmap') ?>
				</dt>
				<dd class="editable">
					<div class="<?= (isset($messages['message'])  ? 'has-error' : '') ?>">
						<textarea name="complex-unit-inquiry[message]" rows="7" class="form-control" placeholder="<?php _e('Describe your inquiry', 'casasyncmap') ?>"><?= $data['message'] ?></textarea>	
					</div>
				</dd>
			</dl>
			
			<button type="submit" class="btn btn-primary pull-right"><?php _e('Send', 'casasyncmap') ?></button>
		</div>
	</div>
</form> 