...
class render extends Feature {
	...
	public function getFormMessages(){
		$defaults = array(
			'first_name' => __('First name is required', 'casasyncmap'),
			'last_name' => __('Last name is required', 'casasyncmap'),
			'email' => __('Email is not valid', 'casasyncmap'),
			'phone' => __('A phone number is required', 'casasyncmap'),
			'street' => __('A street address is required', 'casasyncmap'),
			'postal_code' => __('ZIP is required', 'casasyncmap'),
			'locality' =>  __('City is required', 'casasyncmap'),
			'subject' => '',
			'message' => '',
			'unit_id' => '',
			'post' => __('Ivalid post', 'casasyncmap'),
			'gender' => ''
		);
	...
	public function sendEmail($to = false, $inquiry){
		...
		if ($emails) {
			...
			if (get_cxm($inquiry->ID, 'phone')) {
				$html_contact_data .= __('Phone:', 'casasyncmap') . ' ' . get_cxm($inquiry->ID, 'phone'). '<br>';
			}
			...
			if (get_cxm($inquiry->ID, 'unit')) {
				...
				$html_contact_data .= '<br><br><strong>'.__('Unit', 'casasyncmap').':</strong> '. get_cxm($unit->ID, 'name');
			}
	...
	public function renderForm(){
		...
		if ($formData['post']) {
			if ($this->formValid()) {
				$msg = __('Inquiry has been sent. Thank you!', 'casasyncmap');
				$state = 'success';
				...
				if ($inq_post_id) {
					foreach ($formData as $key => $value) {
						add_post_meta( $inq_post_id, '_casasyncmap_inquiry_'.$key, $value , true);
					}
				}
				...
				$msg = __('Please check the following and try again:', 'casasyncmap');
				$msg .= '<ul>';
